...
export default function ChartsPage() {
  ...
  const processed = useMemo(() => {
    ...
    const sma20Data: LineData[] = calcSMA(closes, 20)
      .map((v, i) => (v !== null ? { time: times[i], value: v } : null))
      .filter(Boolean) as LineData[];
    const ema9Data: LineData[] = calcEMA(closes, 9)
      .map((v, i) => (v !== null ? { time: times[i], value: v } : null))
      .filter(Boolean) as LineData[];
    const bbData = calcBB(closes);
    const bbUpper: LineData[] = bbData
      .map((b, i) => (b.upper !== null ? { time: times[i], value: b.upper } : null))
      .filter(Boolean) as LineData[];
    const bbMid: LineData[] = bbData
      .map((b, i) => (b.mid !== null ? { time: times[i], value: b.mid } : null))
      .filter(Boolean) as LineData[];
    const bbLower: LineData[] = bbData
      .map((b, i) => (b.lower !== null ? { time: times[i], value: b.lower } : null))
      .filter(Boolean) as LineData[];
    const rsiData: LineData[] = calcRSI(closes)
      .map((v, i) => (v !== null ? { time: times[i], value: v } : null))
      .filter(Boolean) as LineData[];
  ...
  useEffect(() => {
    ...
    if (overlays.bb) {
      const u = mc.addSeries(LineSeries, {
        color: C.bbUpper,
        lineWidth: 1,
        lineStyle: 2,
        title: 'BB Upper',
      });
      const m = mc.addSeries(LineSeries, {
        color: C.bbMid,
        lineWidth: 1,
        lineStyle: 3,
        title: 'BB Mid',
      });
      const l = mc.addSeries(LineSeries, {
        color: C.bbLower,
        lineWidth: 1,
        lineStyle: 2,
        title: 'BB Lower',
    ...
    if (overlays.rsi && rsiRef.current) {
      ...
      // OB/OS bands
      const ob70 = rc.addSeries(LineSeries, {
        color: 'rgba(239,68,68,0.5)',
        lineWidth: 1,
        lineStyle: 2,
      });
      ob70.setData(processed.rsiData.map((d) => ({ time: d.time, value: 70 })));
      const os30 = rc.addSeries(LineSeries, {
  ...
  const toggleOverlay = (k: OverlayKey) => setOverlays((prev) => ({ ...prev, [k]: !prev[k] }));
      ...
      <div className="shrink-0 border-b border-[#1c1c1c] bg-[#0d0d0d]">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 flex-wrap">
                ...
                {searchResults.map((r, i) => (
                  ...
                  >
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-[#555] truncate max-w-[180px]">
                        {r.name}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="text-[9px] font-mono text-[#444]">{r.exchange}</span>
                      <span className="text-[9px] font-mono px-1 py-0.5 border border-[#2a2a2a] rounded text-[#555]">
                        {r.type}
                      </span>
                    </div>
                  </button>
          ...
          <button
            ...
            className="h-6 px-1.5 text-[10px] font-mono border border-[#2a2a2a] rounded-sm text-[#555] hover:text-primary hover:border-primary transition-colors"
          >
      ...
      {isError && (
        <div className="flex-1 flex items-center justify-center font-mono text-center">
          <div>
            ...
            <button
              onClick={() => refetch()}
              className="mt-4 px-4 py-1.5 text-xs border border-[#333] rounded-sm text-[#888] hover:border-primary hover:text-primary transition-colors"
            >
              RETRY
            </button>
          </div>
