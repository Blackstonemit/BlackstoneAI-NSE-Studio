...
import { useGetMarketHistory, useGetOptionsChain } from '@workspace/api-client-react';
import { useLiveRefresh } from '@/hooks/use-live-refresh';
import { cn } from '@/lib/utils';
import { formatISTTime } from '@/lib/market-hours';
import { Zap, TrendingUp, TrendingDown, Minus, RefreshCw, Activity } from 'lucide-react';
...
function scoreIndicators(ind: ScalpIndicators): {
  score: number;
  rows: { label: string; value: string; bias: SignalBias }[];
} {
  ...
  if (ind.ema9 !== null && ind.ema21 !== null) {
    ...
    if (emaUp) score += 1;
    else if (emaDown) score -= 1;
    rows.push({
      label: 'EMA 9/21',
      value: `${ind.ema9.toFixed(1)} / ${ind.ema21.toFixed(1)}`,
      bias,
    });
  }
  ...
  if (ind.macd !== null && ind.macdSignal !== null) {
    ...
    rows.push({
      label: 'MACD',
      value: `${ind.macd.toFixed(2)} / ${ind.macdSignal.toFixed(2)}`,
      bias,
    });
  }
  ...
  if (ind.stochK !== null && ind.stochD !== null) {
    ...
    rows.push({
      label: 'Stoch %K/%D',
      value: `${ind.stochK.toFixed(0)} / ${ind.stochD.toFixed(0)}`,
      bias,
    });
  }
...
function getOverallSignal(score: number): {
  label: string;
  color: string;
...
export default function ScalpingPage() {
  ...
  const processed = useMemo(() => {
    ...
    const times = candles.map(
      (c) => (Math.floor(new Date(c.timestamp).getTime() / 1000) + IST_OFFSET_S) as UTCTimestamp,
    );
  ...
  useEffect(() => {
    ...
    const vw = mc.addSeries(LineSeries, {
      color: C.vwap,
      lineWidth: 1,
      lineStyle: 3,
      title: 'VWAP',
    ...
    if (showMACD && macdRef.current) {
      ...
      const sl = rc.addSeries(LineSeries, {
        color: C.macdSig,
        lineWidth: 1,
        lineStyle: 2,
        title: 'Signal',
  ...
  const [searchResults, setSearchResults] = useState<
    { symbol: string; name: string; exchange: string; type: string; yahooSymbol: string }[]
  >([]);
  ...
  const optSnap = useMemo(() => {
    ...
    const ce: OptionContract[] = (chain.calls ?? [])
      .slice()
      .sort((a, b) => b.openInterest - a.openInterest);
    const pe: OptionContract[] = (chain.puts ?? [])
      .slice()
      .sort((a, b) => b.openInterest - a.openInterest);
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 flex-wrap">
          ...
          {last && (
            <div className="ml-2 flex items-center gap-2 font-mono">
              ...
              >
                ...
                {chg >= 0 ? '+' : ''}
                {chg.toFixed(2)} ({processed?.chgPct.toFixed(2)}%)
              </span>
          ...
          <div className="ml-auto flex items-center gap-2 font-mono text-[10px] text-[#555]">
            ...
            <button
              ...
              className={cn(
                'px-1.5 py-0.5 border rounded-sm transition-colors',
                showMACD
      ...
      <div className="flex flex-1 min-h-0">
          ...
          {isLoading ? (
            ...
            <>
              ...
              {showMACD && (
                <div
                  ref={macdRef}
                  className="shrink-0 border-t border-[#1a1a1a]"
                  style={{ height: 100 }}
                />
              )}
            </>
        ...
        <div className="w-72 shrink-0 overflow-y-auto bg-[#0a0a0a] flex flex-col">
          ...
          <div className="p-3 border-b border-[#1a1a1a]">
            ...
            {overall ? (
              <div
                className={cn('flex items-center gap-2 border rounded-sm px-3 py-2', overall.color)}
              >
            ...
            <div className="p-3 border-b border-[#1a1a1a]">
              ...
              <div className="space-y-1">
                {[
                  {
                    label: 'ATR Stop',
                    value: processed.indicators.atr
                      ? (processed.indicators.close - processed.indicators.atr).toFixed(1)
                      : '–',
                    sub: `ATR: ${processed.indicators.atr?.toFixed(1) ?? '–'}`,
                  },
                  {
                    label: 'BB Upper',
                    value: processed.indicators.bbUpper?.toFixed(1) ?? '–',
                    sub: 'resistance',
                  },
                  {
                    label: 'BB Lower',
                    value: processed.indicators.bbLower?.toFixed(1) ?? '–',
                    sub: 'support',
                  },
                  {
                    label: 'VWAP',
                    value: processed.indicators.vwap?.toFixed(1) ?? '–',
                    sub: 'intraday pivot',
                  },
                  {
                    label: 'EMA 21',
                    value: processed.indicators.ema21?.toFixed(1) ?? '–',
                    sub: 'dynamic S/R',
                  },
                ].map((r) => (
          ...
          <div className="p-3 flex-1">
            <div className="text-[10px] font-mono text-[#555] mb-2">
              OPTIONS SNAPSHOT · {preset.optSymbol}
            </div>
            {optSnap ? (
                ...
                <div className="grid grid-cols-3 gap-1 text-center text-[10px] font-mono border border-[#1c1c1c] rounded-sm overflow-hidden">
                  ...
                  <div className="bg-[#0d0d0d] p-1.5 border-x border-[#1c1c1c]">
                    ...
                    <div className="font-bold text-[#aaa]">
                      {optSnap.maxPainStrike.toLocaleString('en-IN')}
                    </div>
                  </div>
                  <div className="bg-[#0d0d0d] p-1.5">
                    ...
                    <div
                      className={cn(
                        'font-bold',
                        optSnap.pcr > 1 ? 'text-emerald-400' : 'text-red-400',
                      )}
                    >
                ...
                <div>
                  ...
                  <div className="h-1.5 rounded-full overflow-hidden bg-[#1c1c1c] flex">
                    <div
                      ...
                      style={{
                        width: `${(optSnap.totalCE / (optSnap.totalCE + optSnap.totalPE)) * 100}%`,
                      }}
                    />
