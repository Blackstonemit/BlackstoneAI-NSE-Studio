...
function bsTheta(
  ...
  const term2 =
    type === 'CE' ? -r * K * Math.exp(-r * T) * ncdf(d2) : r * K * Math.exp(-r * T) * ncdf(-d2);
...
const STRATEGIES: Record<string, Strategy> = {
  ...
  bullCallSpread: {
    ...
    legs: ([k1, k2]) => [
      { strike: k1, type: 'CE', direction: 1 },
      { strike: k2, type: 'CE', direction: -1 },
    ],
  },
  bearPutSpread: {
    name: 'Bear Put Spread',
    legs: ([k1, k2]) => [
      { strike: k2, type: 'PE', direction: 1 },
      { strike: k1, type: 'PE', direction: -1 },
    ],
  },
};

function isSingleLeg(key: string) {
  return ['longCall', 'shortCall', 'longPut', 'shortPut'].includes(key);
}
...
export default function BacktestPage() {
  ...
  const periodDays =
    Math.ceil((new Date(expiryDate).getTime() - new Date(entryDate).getTime()) / 86400000) + 5;
  const period =
    periodDays <= 7
      ? '5d'
      : periodDays <= 30
  ...
  const results = useMemo(() => {
    ...
    const entryT =
      (expiry.getTime() - new Date(entryCandle.timestamp).getTime()) / (365 * 86400000);
    ...
    const entryDelta = legs.reduce(
      (sum, leg) =>
        sum + leg.direction * bsDelta(entrySpot, leg.strike, entryT, r, sigma, leg.type),
      0,
    );
    const entryTheta = legs.reduce(
      (sum, leg) =>
        sum + leg.direction * bsTheta(entrySpot, leg.strike, entryT, r, sigma, leg.type),
      0,
    );
    ...
    const chartData = candles.map((c) => {
      ...
      const T = Math.max(
        0,
        (expiry.getTime() - new Date(c.timestamp).getTime()) / (365 * 86400000),
      );
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                className="font-mono uppercase bg-background border-muted"
                placeholder="NIFTY"
              />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-mono text-muted-foreground">
                {needsStrike2 ? 'STRIKE 1 (BUY)' : 'STRIKE'}
              </label>
              <Input
                value={strike1}
                onChange={(e) => setStrike1(e.target.value)}
                type="number"
                className="font-mono bg-background border-muted"
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              {
                label: 'ENTRY PREMIUM',
                value: `₹${fmt(results.entryPremium)}`,
                sub: `${results.multiplier} units`,
          ...
          <Card className="rounded-sm border-muted bg-card">
            <CardContent className="p-4">
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-xs font-mono">
                <div>
                  <div className="text-muted-foreground mb-1">ENTRY SPOT</div>
                  <div className="font-bold text-lg">₹{fmt(results.entrySpot)}</div>
                </div>
                <div>
                  <div className="text-muted-foreground mb-1">DELTA</div>
                  <div className="font-bold text-lg">{fmt(results.entryDelta)}</div>
                </div>
                <div>
                  <div className="text-muted-foreground mb-1">THETA / DAY</div>
                  <div className="font-bold text-lg text-destructive">
                    {fmt(results.entryTheta)}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground mb-1">BREAKEVEN</div>
                  <div className="font-bold text-lg">
                    {results.breakeven ? `₹${fmt(results.breakeven)}` : '—'}
                  </div>
                </div>
                <div>
                ...
                >
                  <defs>
                    <linearGradient id="pnlGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop
                        offset="5%"
                        stopColor={results.finalPnl >= 0 ? '#22c55e' : '#ef4444'}
                        stopOpacity={0.3}
                      />
                      <stop
                        offset="95%"
                  ...
                  <Area
                    type="monotone"
                    dataKey="pnl"
                    stroke={results.finalPnl >= 0 ? '#22c55e' : '#ef4444'}
                    strokeWidth={2}
                    fill="url(#pnlGrad)"
                    dot={false}
                  />
                ...
                <table className="w-full text-xs font-mono">
                  <thead>
                    <tr className="border-b border-muted bg-muted/20">
                      ...
                      <th className="py-2 px-4 text-right text-muted-foreground font-normal">
                        SPOT
                      </th>
                      <th className="py-2 px-4 text-right text-muted-foreground font-normal">
                        OPTION VALUE
                  ...
                  <tbody>
                    {results.chartData.map((row, i) => (
                      <tr key={i} className="border-b border-muted/30 hover:bg-muted/10">
                        ...
                        <td
                          className={cn(
                            'py-1.5 px-4 text-right font-bold',
