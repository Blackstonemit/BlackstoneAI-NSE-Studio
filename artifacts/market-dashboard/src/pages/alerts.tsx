...
const STATUS_CONFIG = {
  ACTIVE: {
    label: 'Active',
    color: 'bg-blue-500/10 text-blue-400 border-blue-800/30',
    icon: Clock,
  },
  TRIGGERED: {
    label: 'Triggered',
    color: 'bg-green-500/10 text-green-400 border-green-800/30',
    icon: CheckCircle2,
  },
  CANCELLED: {
    label: 'Cancelled',
...
export default function AlertsPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.symbol.trim() || isNaN(price) || price <= 0) {
      toast({
        title: 'Invalid input',
        description: 'Enter a valid symbol and price',
        variant: 'destructive',
      });
      return;
    }
  ...
  return (
    <div className="space-y-6">
                      ...
                      {group.items.map((alert) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <span
                                className={`flex items-center gap-1 ${alert.condition === 'ABOVE' ? 'text-green-400' : 'text-red-400'}`}
                              >
                            ...
                            <td className="px-4 py-3">
                              <span
                                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm border text-[10px] ${cfg.color}`}
                              >
      ...
      <p className="text-xs text-muted-foreground/60 border-t border-border pt-3">
        ⚠ Alerts are checked on a polling basis. Trigger accuracy depends on data refresh intervals.
