  ...
  return (
    <div className="flex items-center gap-3 text-[11px] font-mono text-muted-foreground border border-muted/40 rounded-sm px-3 py-1.5 bg-muted/5">
      ...
      <span>
        ...
        <span
          className={
            settings.agentAutoGenerate ? 'text-green-400 font-bold' : 'text-muted-foreground/50'
          }
        >
...
export default function SignalsBoard() {
  ...
  const handleSchedulerExpire = async () => {
    try {
      ...
      toast({
        title: 'Expiry Complete',
        description: `${data.expired} signal(s) marked as EXPIRED.`,
      });
      queryClient.invalidateQueries({ queryKey: getGetSignalsQueryKey() });
    } catch {
    ...
    try {
      ...
      if (data.generated > 0) {
        toast({
          ...
          description: `${data.generated} new signal(s) created by AI scheduler.`,
        });
        queryClient.invalidateQueries({ queryKey: getGetSignalsQueryKey() });
      } else {
        toast({
          title: 'No Signals Generated',
          description: 'Market may be closed or AI returned no results.',
          variant: 'destructive',
        });
      }
    } catch {
      ...
      <div className="grid grid-cols-1 gap-4">
        {isLoading ? (
          ...
          signals.map((signal) => (
            <Card
              key={signal.id}
              className="rounded-sm border-muted bg-card hover:border-primary/50 transition-colors"
            >
          ...
          <div className="py-20 text-center border border-muted border-dashed rounded-sm">
            ...
            <Button onClick={handleGenerate} variant="outline" className="font-mono">
