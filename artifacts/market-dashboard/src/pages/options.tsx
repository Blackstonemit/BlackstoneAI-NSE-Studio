...
function parseNseCsv(content: string): CsvRow[] {
  ...
  for (const line of dataLines) {
    ...
    rows.push({
      ...
      put: {
        oi: parseNum(cols[21]),
        chgOI: parseNum(cols[20]),
        vol: parseNum(cols[19]),
        iv: parseNum(cols[18]),
        ltp: parseNum(cols[17]),
        chg: parseNum(cols[16]),
        bid: parseNum(cols[13]),
        ask: parseNum(cols[14]),
      },
    });
  }
  return rows;
...
export default function OptionsChain() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetOptionsChain(queryParams, {
  ...
  const csvStats = useMemo(() => {
    ...
    return {
      ...
      avgCallLtpDiff: matched > 0 ? totalCallLtpDiff / matched : 0,
      avgPutLtpDiff: matched > 0 ? totalPutLtpDiff / matched : 0,
      csvCallOI,
      csvPutOI,
      liveCallOI,
      livePutOI,
    ...
    chainData.calls.forEach((call) => {
      ...
      if (isNSE) {
        rows.push(
          [
            call.openInterest,
            (call as NseContract).changeInOI ?? 0,
            call.volume,
            call.impliedVolatility.toFixed(2),
            (call as NseContract).bid ?? 0,
            (call as NseContract).ask ?? 0,
            call.ltp.toFixed(2),
            call.change.toFixed(2),
            call.strikePrice,
            put.change.toFixed(2),
            put.ltp.toFixed(2),
            (put as NseContract).bid ?? 0,
            (put as NseContract).ask ?? 0,
            put.impliedVolatility.toFixed(2),
            put.volume,
            (put as NseContract).changeInOI ?? 0,
            put.openInterest,
          ].join(','),
        );
      } else {
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          {csvRows ? (
            <div className="flex items-center gap-1">
              <button
                onClick={() => setViewMode(viewMode === 'compare' ? 'live' : 'compare')}
                className={cn(
        ...
        <div className="space-y-4">
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-0 divide-x divide-muted border-b border-muted text-center text-xs font-mono">
                    {[
                      {
                        label: 'LIVE PCR',
                        val: csvStats.livePCR.toFixed(2),
                        color: csvStats.livePCR > 1 ? 'text-green-400' : 'text-red-400',
                      },
                      {
                        label: 'CSV PCR',
                        val: csvStats.csvPCR.toFixed(2),
                        color: csvStats.csvPCR > 1 ? 'text-green-400' : 'text-red-400',
                      },
                      {
                        label: 'LIVE CALL OI',
                        val: fmtIn(csvStats.liveCallOI),
                        color: 'text-blue-400',
                      },
                      {
                        label: 'CSV CALL OI',
                        val: fmtIn(csvStats.csvCallOI),
                        color: 'text-blue-300',
                      },
                      {
                        label: 'AVG LTP DIFF',
                        val: `CALL ₹${csvStats.avgCallLtpDiff.toFixed(2)} / PUT ₹${csvStats.avgPutLtpDiff.toFixed(2)}`,
                    ...
                    >
                      <thead>
                        <tr className="border-b border-muted bg-muted/20 text-muted-foreground">
                          <th
                            className="py-1.5 px-3 text-center bg-blue-900/20 text-blue-300 font-normal"
                            colSpan={3}
                          >
                            CALL LTP
                          </th>
                          ...
                          <th className="py-1.5 px-3 text-center bg-muted/30 text-foreground font-bold">
                            STRIKE
                          </th>
                          <th
                            className="py-1.5 px-3 text-center bg-red-900/20 text-red-300 font-normal"
                            colSpan={2}
                          >
                            PUT OI
                          </th>
                        ...
                        <tr className="border-b-2 border-muted text-muted-foreground bg-muted/10 text-[10px]">
                          ...
                          <th className="py-1 px-3 text-right font-normal text-blue-300">LIVE</th>
                          <th className="py-1 px-3 text-right font-normal border-r border-muted">
                            CSV
                          </th>
                          <th className="py-1 px-3 text-center font-bold border-x border-muted bg-muted/30" />
                          <th className="py-1 px-3 text-left font-normal border-l border-muted text-red-300">
                            LIVE
                          </th>
                      ...
                      <tbody>
                        {(() => {
                          const csvMap = new Map(csvRows.map((r) => [r.strike, r]));
                          const putsByStrikeCompare = new Map(
                            chainData.puts.map((p) => [p.strikePrice, p]),
                          );
                          return chainData.calls.map((call, i) => {
                            ...
                            return (
                              ...
                              >
                                <td className="py-1.5 px-3 text-right tabular-nums text-blue-300 font-bold">
                                  {call.ltp.toFixed(2)}
                                </td>
                                <td className="py-1.5 px-3 text-right tabular-nums text-muted-foreground">
                                  {csv?.call.ltp?.toFixed(2) ?? '—'}
                                </td>
                                <td className="py-1.5 px-3 text-right">
                                  <DiffBadge live={call.ltp} csv={csv?.call.ltp ?? null} />
                                </td>
                                <td className="py-1.5 px-3 text-right tabular-nums text-muted-foreground">
                                  {fmtOI(call.openInterest)}
                                </td>
                                <td className="py-1.5 px-3 text-right tabular-nums text-muted-foreground border-r border-muted">
                                  {csv?.call.oi != null ? fmtOI(csv.call.oi) : '—'}
                                </td>
                                <td
                                  className={cn(
                                    'py-1.5 px-3 text-center font-bold border-x border-muted',
                                    isATM ? 'bg-yellow-500/20 text-yellow-300' : 'bg-muted/20',
                                  )}
                                >
                                  {isATM && (
                                    <span className="text-[9px] block text-yellow-400 leading-none">
                                      ATM
                                    </span>
                                  )}
                                  {call.strikePrice}
                                </td>
                                <td className="py-1.5 px-3 text-left tabular-nums text-muted-foreground border-l border-muted">
                                  {fmtOI(put.openInterest)}
                                </td>
                                <td className="py-1.5 px-3 text-left tabular-nums text-muted-foreground">
                                  {csv?.put.oi != null ? fmtOI(csv.put.oi) : '—'}
                                </td>
                                <td className="py-1.5 px-3 text-left tabular-nums text-red-300 font-bold">
                                  {put.ltp.toFixed(2)}
                                </td>
                                <td className="py-1.5 px-3 text-left tabular-nums text-muted-foreground">
                                  {csv?.put.ltp?.toFixed(2) ?? '—'}
                                </td>
                                <td className="py-1.5 px-3 text-left">
                                  <DiffBadge live={put.ltp} csv={csv?.put.ltp ?? null} />
                                </td>
                              </tr>
                            );
                          });
          ...
          <div className="border border-muted rounded-sm bg-card overflow-hidden">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 border-b border-muted divide-x divide-muted text-center text-xs font-mono">
              {[
                ...
                { label: 'PUT VOL', value: fmtIn(stats!.totalPutVol), color: 'text-red-300' },
                {
                  label: 'MAX OI',
                  value: null,
                  callStrike: stats!.maxCallOIStrike,
                  putStrike: stats!.maxPutOIStrike,
                },
              ].map((s, i) => (
            ...
            <div className="overflow-x-auto">
              <table
                className="w-full text-xs font-mono border-collapse"
                style={{ minWidth: isNSE ? '1100px' : '700px' }}
              >
                <thead>
                  <tr>
                    <th
                  ...
                  <tr className="border-b-2 border-muted text-muted-foreground bg-muted/20 text-[11px]">
                    ...
                    <th className="py-1.5 px-2 text-right font-normal border-r border-muted bg-blue-900/30 text-blue-300">
                      LTP
                    </th>
                    <th className="py-1.5 px-2 text-center font-bold border-x border-muted bg-muted/30" />
                    <th className="py-1.5 px-2 text-left font-normal border-l border-muted bg-red-900/30 text-red-300">
                      LTP
                    </th>
                ...
                <tbody>
                  {(() => {
                    const putsByStrikeLive = new Map(
                      chainData.puts.map((p) => [p.strikePrice, p as NseContract]),
                    );
                    return chainData.calls.map((call, i) => {
                        ...
                        >
                          <td
                            className={cn(
                              ...
                              isCallITM ? 'bg-blue-500/8' : '',
                            )}
                          >
                            <span
                              className={
                                isMaxCallOI ? 'text-blue-400 font-bold' : 'text-muted-foreground'
                              }
                            >
                              {isMaxCallOI && '★ '}
                              {fmtOI(call.openInterest)}
                          ...
                          >
                            {isATM && (
                              <span className="text-[9px] block text-yellow-400 leading-none mb-0.5">
                                ATM
                              </span>
                            )}
                            {call.strikePrice}
                            <div className="flex items-center justify-center gap-1 mt-0.5">
                              <TradeButtons
                                symbol={`${symbol}${call.strikePrice}CE`}
                                price={call.ltp}
                              />
                              <span className="text-[9px] text-muted-foreground/30">|</span>
                              <TradeButtons
                                symbol={`${symbol}${put.strikePrice}PE`}
                                price={put.ltp}
                              />
                            </div>
                          </td>
                          ...
                          {isNSE && (
                            <td
                              className={cn(
                                'py-1.5 px-2 text-left tabular-nums text-muted-foreground',
                                isPutITM ? 'bg-red-500/8' : '',
                              )}
                            >
                          ...
                          >
                            <span
                              className={
                                isMaxPutOI ? 'text-red-400 font-bold' : 'text-muted-foreground'
                              }
                            >
                              {isMaxPutOI && '★ '}
                              {fmtOI(put.openInterest)}
