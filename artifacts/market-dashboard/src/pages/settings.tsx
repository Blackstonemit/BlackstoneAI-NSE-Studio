...
const PROVIDER_META: Record<
  string,
  { label: string; color: string; keyUrl: string; keyHint: string; model: string }
> = {
  nvidia: {
    label: 'NVIDIA NIM',
...
function AIProvidersCard() {
  ...
  const fetchStatus = async () => {
    try {
      ...
      toast({
        title: 'Error',
        description: 'Could not load AI provider status.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleSaveKey = async (provider: 'openai' | 'claude' | 'gemini') => {
    ...
    try {
      ...
      toast({
        title: 'API Key Saved',
        description: `${PROVIDER_META[provider].label} is now active as a fallback.`,
      });
    ...
    <Card className="rounded-sm border-primary/20 bg-card">
      ...
      <CardContent className="p-4 space-y-4">
        <p className="text-xs text-muted-foreground font-mono">
          When the primary provider fails, the system automatically tries the next configured one in
          order.
        </p>
                ...
                <div
                  ...
                  className={cn(
                    ...
                    p.configured
                      ...
                      : 'border-muted/40 bg-muted/5 opacity-70',
                  )}
                >
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-mono font-bold text-muted-foreground/50 w-5">
                      #{idx + 1}
                    </span>
                    <div className={cn('font-mono font-bold text-sm', meta.color)}>
                      {meta.label}
                    </div>
                  ...
                  {!isNvidia && (
                    <form
                      ...
                      onSubmit={(e) => {
                        e.preventDefault();
                        handleSaveKey(p.provider as 'openai' | 'claude' | 'gemini');
                      }}
                    >
                      <div className="relative flex-1">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setShowKey((prev) => ({ ...prev, [p.provider]: !prev[p.provider] }))
                          }
...
function StyleBadge({ style }: { style: string }) {
  ...
  return (
    <span
      className={cn(
        'text-[10px] px-1.5 py-0.5 border rounded-full font-mono font-bold uppercase tracking-wider',
        colors[style] ?? 'bg-muted/20 text-muted-foreground',
      )}
    >
      {style}
    </span>
        ...
        <Card className="rounded-sm border-muted bg-card">
          ...
          <CardContent className="p-4">
            ...
            <Row label="Default Exchange">
              <Select
                value={local.defaultExchange}
                onValueChange={(v: 'NSE' | 'BSE') => set('defaultExchange', v)}
              >
          ...
          <CardContent className="p-4">
            <Row label="Default Timeframe" hint="Default timeframe for signal generation">
              <Select
                ...
                onValueChange={(v: 'INTRADAY' | 'SWING' | 'POSITIONAL') =>
        ...
        <Card className="rounded-sm border-muted bg-card">
          ...
          <CardContent className="p-4">
            <Row
              ...
              hint="Show synthetic options data when live NSE feed is unavailable"
            >
              <Toggle
                checked={local.showSyntheticData}
                onChange={(v) => set('showSyntheticData', v)}
              />
            </Row>
            <Row
              label="Highlight ATM Strike"
              hint="Visually highlight the At-The-Money strike in options chain"
            >
      ...
      <Card className="rounded-sm border-primary/30 bg-card">
        ...
        <CardContent className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
            ...
            <div>
              <Row
                label="Default Instrument Type"
                hint="Type of instrument the agent analyses by default"
              >
                <Select
                  value={local.agentInstrumentType}
                  onValueChange={(v: typeof local.agentInstrumentType) =>
                    set('agentInstrumentType', v)
                  }
                >
                  <SelectTrigger className="w-36 font-mono border-muted bg-background text-sm">
                    <SelectValue />
              ...
              <Row label="Agent Timeframe" hint="Default analysis horizon for agent signals">
                <Select
                  value={local.agentTimeframe}
                  onValueChange={(v: typeof local.agentTimeframe) => set('agentTimeframe', v)}
                >
                  <SelectTrigger className="w-36 font-mono border-muted bg-background text-sm">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              <Row
                ...
                hint="Only show signals above this confidence level"
              >
                <div className="flex items-center gap-3">
                  <Slider
                    min={0}
                  ...
                  <span
                    className={cn(
                      'font-mono font-bold text-sm w-8',
                      local.agentConfidenceThreshold >= 70
                        ? 'text-green-400'
                        : local.agentConfidenceThreshold >= 40
                          ? 'text-yellow-400'
                          : 'text-muted-foreground',
                    )}
                  >
                    {local.agentConfidenceThreshold}%
                  </span>
                </div>
              </Row>
              <Row
                label="Auto-Run on Page Load"
                hint="Automatically run agent analysis when opening Technical Analysis"
              >
                <Toggle checked={local.agentAutoRun} onChange={(v) => set('agentAutoRun', v)} />
              </Row>
              <Row
                label="Save Signals to DB"
                hint="Persist AI-generated signals to the signals board"
              >
                <Toggle
                  checked={local.agentSaveSignals}
                  onChange={(v) => set('agentSaveSignals', v)}
                />
              </Row>
              <Row
                label="Auto-Generate Signals"
                hint="Server auto-generates signals every 30 min during market hours"
              >
                <Toggle
                  checked={local.agentAutoGenerate}
                  onChange={(v) => set('agentAutoGenerate', v)}
                />
              </Row>
            </div>
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-muted/40 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground font-mono">
                (optional — injected into every agent prompt)
              </span>
            </div>
            <Textarea
      ...
      <Card className="rounded-sm border-muted/40 bg-muted/10">
        <CardContent className="p-4">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-xs font-mono">
            <div>
              <div className="text-muted-foreground mb-1">SYMBOL / EXCH</div>
              <div className="font-bold">
                {local.defaultSymbol} / {local.defaultExchange}
              </div>
            </div>
            <div>
              <div className="text-muted-foreground mb-1">REFRESH</div>
              <div className="font-bold">
                {local.autoRefresh ? `Every ${local.refreshInterval / 1000}s` : 'PAUSED'}
              </div>
            </div>
            <div>
            ...
            <div>
              <div className="text-muted-foreground mb-1">AGENT STYLE</div>
              <div className="font-bold">
                <StyleBadge style={local.agentStyle} />
              </div>
            </div>
            <div>
              <div className="text-muted-foreground mb-1">MIN CONFIDENCE</div>
              <div className="font-bold">{local.agentConfidenceThreshold}%</div>
            </div>
          </div>
        </CardContent>
      </Card>
